...
public class RangeSlashRenderer extends GeoEntityRenderer<EntityRangeSlash> {
	private static final ResourceLocation TEXTURE = new ResourceLocation(InvasionCodeRed.MODID,
			"textures/entity/rangeslash.png");
	private static final ResourceLocation TEXTURE_BIG = new ResourceLocation(InvasionCodeRed.MODID,
			"textures/entity/rangeslash_big.png");
	...
	public void render(EntityRangeSlash animatable, float entityYaw, float partialTick, PoseStack poseStack,
		...
		poseStack.mulPose(new Quaternionf()
				.rotationY(Mth.lerp(partialTick, animatable.yRotO, animatable.getYRot()) * ((float) Math.PI / 180F)));
		poseStack.mulPose(new Quaternionf()
				.rotationZ(Mth.lerp(partialTick, animatable.xRotO, animatable.getXRot()) * ((float) Math.PI / 180F)));
